...
export const posts: Post[] = [
  {
    ...
    content: `지난주 토요일, 해시드 바이브랩스 서울 에디션의 심사를 마쳤다. 이후 모든 지원자에게 결과를 안내하는 메일을 보냈다. 합격한 분들보다 함께하지 못하는 분들이 훨씬 많았다. 그 숫자가 쉽게 넘어가지 않았다.
...
❤43👍2`,
...

  {
    ...
    content: `컴퓨터라는 단어가 처음 등장한 건 1613년이다. 그것은 기계의 이름이 아니었다. 직업명이었다. 수식을 계산하는 사람, 천문학자 옆에서 숫자를 검산하는 사람을 가리켰다. 요하네스 케플러도 컴퓨터를 고용했다. 앨런 튜링은 300년 뒤에 그 이름을 딴 기계를 설계하면서 이렇게 정의했다. "고정된 규칙을 따르는 인간." 기계는 처음부터 사람의 이름으로 태어났다. 그런데 지금의 AI는 더 이상 고정된 규칙을 따르지 않는다. 규칙을 스스로 발견하고 때로는 새로 만든다. 튜링의 정의가 완전히 뒤집히고 있다. 그렇다면 규칙을 만드는 존재가 기계가 된 시대에, 인간은 무엇으로 정의되어야 하는가.
