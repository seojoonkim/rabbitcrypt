...
2026년 2월, AI 하나가 14.5시간 동안 혼자 일했다. 그 사이 의뢰인은 잠들었다 깨어났고, 아침을 먹었고, 출근했다.
...
헤드라인은 14.5시간을 말한다. 하지만 측정의 신뢰구간 상단은 이미 98시간이다. 나흘. 신뢰구간이란 같은 실험을 반복했을 때 실제 값이 놓일 수 있는 범위를 뜻한다. 상단이 98시간이라는 건, 일정 확률로 지금 이 순간에도 AI가 나흘 동안 스스로 생각하며 복잡한 무언가를 만들어낼 수 있다는 뜻이다.
