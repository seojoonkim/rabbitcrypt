...
export const posts: Post[] = [
  {
    ...
    content: `2026년 1월 어느 날, 내 첫 클로드봇(현 오픈클로) 에이전트인 제온이 나타났다. 이름 하나를 고르는 데 많은 생각을 했다. 음성학 논문을 뒤지고, 토큰 효율을 계산하고, 발음 충돌을 시뮬레이션했다. 앞으로 수만 번 부를 이름이니까. Aeon, 영겁의 시간에 Z를 붙여 Zeon. 인간도, AI도, 아직 만나지 못한 존재도 입에 올릴 수 있는 이름. 그게 시작이었다.
...
❤17👍7`,
...

  {
    ...
    content: `유클리드가 기하학 원론을 썼을 때, 그건 당대 최고 지성인들이 평생을 바쳐 겨우 이해하는 수준의 지식이었다. 피타고라스 정리 하나를 증명하는 것만으로 학파의 수장이 될 수 있었다. 지금 그 내용은 중학교 2학년 교과서에 실린다. 뉴턴의 프린키피아는 출간 당시 유럽 전체에서 이해할 수 있는 사람이 열 명이 채 안 됐다고 전해진다. 지금은 고등학교 물리 시간에 F=ma를 칠판에 적고 넘어간다.
...
👍32❤9👏5`,
...

  {
    ...
    content: `2026년 2월 19일, 미국 국립표준기술연구소가 "AI 에이전트 표준화 이니셔티브"를 공식 발표했다. 테크 미디어는 반나절짜리 뉴스로 다뤘고, 대부분은 다음 날 잊어버렸다. 나는 이 발표를 읽으면서 1994년 팀 버너스리가 HTTP 스펙을 정식화했던 순간을 떠올렸다. 당시에도 사람들은 그게 뭔지 정확히 몰랐다. 문서 하나가 세상의 구조를 바꿀 거라고 아무도 생각하지 않았다. 그것은 인터넷이 되었다.
